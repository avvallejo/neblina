...
export function adaptPedido(p) {
  return {
    ...
    nombreTicket: p.nombre_ticket || [p.cliente_nombre, p.cliente_apellido].filter(Boolean).join(' '),
    levantadoPor: p.levantado_por_nombre || '',
    origen: p.origen,
...
export function adaptTicket(pi) {
  return {
    ...
    terminadoPor: pi.terminado_por_nombre || '',
    finishedAt: pi.terminado_en ? new Date(pi.terminado_en).getTime() : null,
    nombreTicket: pi.nombre_ticket || [pi.cliente_nombre, pi.cliente_apellido].filter(Boolean).join(' '),
    levantadoPor: pi.levantado_por_nombre || '',
    origen: pi.origen,
