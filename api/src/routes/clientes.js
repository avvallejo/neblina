...
const { requireAuth, requireCliente, requireRole, resolveSucursal } = require('../middleware/auth');
...
router.get('/', requireAuth, requireRole('admin'), resolveSucursal, asyncHandler(async (req, res) => {
  const { rows } = await query('SELECT id, nombre, apellido, telefono, pedidos_app_contador, recompensa_pendiente, telefono_verificado, creado_en FROM clientes WHERE sucursal_id = $1 ORDER BY creado_en DESC', [req.sucursalId]);
  res.json(rows);
}));
...
router.post('/', requireAuth, requireRole('cajero', 'admin'), resolveSucursal, asyncHandler(async (req, res) => {
  ...
  const existente = await query('SELECT id FROM clientes WHERE telefono = $1 AND sucursal_id = $2', [telefonoLimpio, req.sucursalId]);
  if (existente.rows.length > 0) throw new ApiError(409, 'Ya existe un cliente con ese teléfono.');

  const { rows } = await query(
    'INSERT INTO clientes (nombre, apellido, telefono, telefono_verificado, sucursal_id) VALUES ($1,$2,$3,true,$4) RETURNING *',
    [nombre.trim(), apellido.trim(), telefonoLimpio, req.sucursalId]
  );
