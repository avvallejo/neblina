...
(async () => {
  ...
  // Cortesías parciales por HTTP: tanto cobro inmediato como ticket abierto.
  r=await req('POST','/pedidos',{destino:'barra',items:[{...line,esCortesia:true},line],pago:{metodoPago:'efectivo',montoRecibido:50}});
  assert.equal(r.status,201,JSON.stringify(r.data));
  assert.equal(Number(r.data.pedido.total),50);
  assert.equal(Number(r.data.pedido.cortesia_unidades),1);
  assert.equal(r.data.pedido.metodo_pago,'efectivo');
  const parcial=await create();
  r=await req('POST',`/pedidos/${parcial.id}/items`,{items:[{...line,esCortesia:true}]});
  assert.equal(r.status,201,JSON.stringify(r.data));
  assert.equal(Number(r.data.total),50);
  r=await req('PATCH',`/pedidos/${parcial.id}/cobrar`,{metodoPago:'efectivo',montoRecibido:50,totalEsperado:50});
  assert.equal(r.status,200,JSON.stringify(r.data));
  assert.equal(Number(r.data.cortesia_unidades),1);
  const gratuito=await create();
  const gratisItems=(await detail(gratuito.id)).items;
  r=await req('PATCH',`/pedidos/${gratuito.id}/cobrar`,{metodoPago:'efectivo',itemsCortesia:gratisItems.map(i=>i.id),totalEsperado:50});
  assert.equal(r.status,200,JSON.stringify(r.data));
  assert.equal(Number(r.data.total),0); assert.equal(r.data.metodo_pago,'cortesia');
  console.log('PASS: cortesías parciales por HTTP, cobro inmediato, agregar a ticket abierto y marcar al cobrar');
